...
def bri_clr_loader1(data):
    img1 = cv2.cvtColor(data, cv2.COLOR_RGB2HSV) #zjq
...
def read_image(file_name, format=None): #img_transform,toPIL,
    """
    ...
    ir_rgb = Image.fromarray(ir_rgb)
    ir_rgb = _apply_exif_orientation(ir_rgb)
    ir_rgb = convert_PIL_to_numpy(ir_rgb, format)
    vi = Image.fromarray(vi)
    vi = _apply_exif_orientation(vi)
    vi = convert_PIL_to_numpy(vi, format)
    visimage_bri, visimage_clr = bri_clr_loader1(vi.copy())
    return ir, ir_rgb, vi, visimage_bri, visimage_clr
