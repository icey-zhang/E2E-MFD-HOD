...
from .adm.utils import utils as utils
# from .adm.benchmarks.mtl_benchmark import get_benchmark_class
__all__ = ["HookBase", "TrainerBase", "SimpleTrainer", "AMPTrainer"]

#从第1000个iter开始对齐，每隔1000对齐一次
...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        """
        loss_dict = self.model(data) # 在这里写对齐
        # shared_parameter = self.model.backbone #共享参数
        shared_parameter = [] #list(self.model.backbone.parameters())
        ...
        for name, p in self.model.backbone.named_parameters():
            ...
            # else:
            #     print(name)
        fusion_parameter = self.model.fusion #各自的参数
        detection_parameter = self.model.head  #检测网络的参数 
        task_specific_params={'0': list(detection_parameter.parameters()),'1':list(fusion_parameter.parameters())}
        
        
        # torch.save(self.model.state_dict(), 'aaaa.pth')
        ...
        if not self.zero_grad_before_forward:
            ...
            """
            self.optimizer.zero_grad()
        if self.iter % 1000 ==0 and self.iter>=1000: #从第1000个iter开始对齐，每隔1000对齐一次
        # if self.iter >= 0 :
