...
class DetcropPixelLoss(nn.Module):
    ...
    def forward(self, im_fusion, im_v, im_tir, mask,inf_weight=None,vis_weight=None):
        """Forward function.
        Args:
            # im_fusion (Tensor): Fusion image with shape (N, C, H, W).
            # im_v (Tensor): HSV(V) image with shape (N, C, H, W).
            # im_tir (Tensor): IR image with shape (N, C, H, W).
        """

        image_ir=im_tir[:,:1,:,:]
        # pixel_max = torch.max(im_v, image_ir)
        pixel_max = im_v + image_ir
